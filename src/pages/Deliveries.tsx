/**
 * DELIVERIES PAGE — Glass UI for outgoing goods
 */
...
const Deliveries: React.FC = () => {
  ...
  const openCreate = () => {
    ...
    setReference(`DEL-${new Date().getFullYear()}-${String(n).padStart(3, '0')}`);
    setCustomer(''); setWarehouseId(warehouses[0]?.id || ''); setLines([]); setFormErrors({}); setShowForm(true);
  };

  const addLine = () => {
    const unused = products.find(p => !lines.some(l => l.productId === p.id));
    if (unused) setLines([...lines, { productId: unused.id, orderedQty: 1, deliveredQty: 0 }]);
  };
  ...
  return (
    <div className="space-y-5">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h1 className="text-xl font-bold" style={{ color: 'var(--text-primary)' }}>Delivery Orders</h1>
          <p className="text-[13px] mt-0.5" style={{ color: 'var(--text-tertiary)' }}>Manage outgoing shipments to customers</p>
        </div>
      ...
      <div className="space-y-3">
        {filtered.map(del => {
          ...
          return (
            <div key={del.id} className="glass-card rounded-2xl p-4 flex flex-col sm:flex-row sm:items-center justify-between gap-3">
              <div className="flex-1">
                <div className="flex items-center gap-2 mb-1">
                  <Truck size={15} className="text-blue-500" />
                  <span className="font-bold text-[14px]" style={{ color: 'var(--text-primary)' }}>{del.reference}</span>
                  <span className={`badge ${statusBadge[del.status]}`}>{del.status}</span>
                </div>
                <div className="text-[13px]" style={{ color: 'var(--text-secondary)' }}>
                  Customer: <span style={{ color: 'var(--text-primary)' }}>{del.customer}</span> · {wh?.name} · {del.lines.length} item(s)
                </div>
                <div className="text-[11px] mt-1" style={{ color: 'var(--text-tertiary)' }}>{new Date(del.createdAt).toLocaleDateString()}</div>
              </div>
              <div className="flex items-center gap-2">
                <button onClick={() => setViewId(del.id)} className="px-3 py-1.5 text-sm rounded-xl btn-secondary inline-flex items-center gap-1"><Eye size={13} /> View</button>
                {del.status !== 'done' && del.status !== 'canceled' && (
                  <>
                    {del.status === 'draft' && (
                      <button onClick={() => updateDeliveryStatus(del.id, 'ready')} className="px-3 py-1.5 text-sm rounded-xl font-medium badge-ready">Pack</button>
                    )}
                    {(del.status === 'waiting' || del.status === 'ready') && (
                      <button onClick={() => validateDelivery(del.id)} className="px-3 py-1.5 text-sm rounded-xl font-medium btn-success inline-flex items-center gap-1"><CheckCircle size={13} /> Validate</button>
                    )}
                  </>
                )}
              </div>
        ...
        {filtered.length === 0 && (
          <div className="text-center py-12" style={{ color: 'var(--text-tertiary)' }}><Truck size={40} className="mx-auto mb-2 opacity-30" />No deliveries found</div>
        )}
      </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4" style={{ background: 'var(--modal-overlay)', backdropFilter: 'blur(8px)' }}>
          <div className="glass-modal rounded-2xl w-full max-w-lg max-h-[90vh] overflow-y-auto animate-scaleIn">
            <div className="flex items-center justify-between px-6 py-4" style={{ borderBottom: '1px solid var(--divider)' }}>
              ...
              <button onClick={() => setViewId(null)} className="btn-ghost p-1.5 rounded-lg"><X size={18} /></button>
            </div>
            <div className="p-6 space-y-4">
              <div className="grid grid-cols-2 gap-4 text-sm">
                <div><span style={{ color: 'var(--text-tertiary)' }}>Customer:</span> <span className="font-medium" style={{ color: 'var(--text-primary)' }}>{viewing.customer}</span></div>
              ...
              <div className="glass rounded-xl overflow-hidden">
                <table className="w-full text-sm glass-table">
                  <thead><tr><th className="text-left px-3 py-2">Product</th><th className="text-right px-3 py-2">Ordered</th><th className="text-right px-3 py-2">Delivered</th></tr></thead>
                  <tbody>
                    {viewing.lines.map((line, idx) => {
                      ...
                      return (
                        <tr key={idx} style={{ borderTop: '1px solid var(--divider)' }}>
                          ...
                          <td className="px-3 py-2 text-right" style={{ color: 'var(--text-secondary)' }}>{line.orderedQty}</td>
                          <td className="px-3 py-2 text-right font-medium" style={{ color: 'var(--text-primary)' }}>{viewing.status === 'done' ? (line.deliveredQty || line.orderedQty) : line.deliveredQty}</td>
                        </tr>
                      );
                    })}
                  </tbody>
      ...
      {showForm && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4" style={{ background: 'var(--modal-overlay)', backdropFilter: 'blur(8px)' }}>
          <div className="glass-modal rounded-2xl w-full max-w-lg max-h-[90vh] overflow-y-auto animate-scaleIn">
            <div className="flex items-center justify-between px-6 py-4" style={{ borderBottom: '1px solid var(--divider)' }}>
              <h2 className="text-lg font-bold" style={{ color: 'var(--text-primary)' }}>New Delivery Order</h2>
            ...
            <form onSubmit={handleSubmit} className="p-6 space-y-4">
              ...
              <div>
                <label className="block text-xs font-medium mb-1.5" style={{ color: 'var(--text-secondary)' }}>Customer *</label>
                <input type="text" value={customer} onChange={e => setCustomer(e.target.value)}
                  className={`w-full px-4 py-2.5 rounded-xl text-sm glass-input ${formErrors.customer ? 'border-red-400/50' : ''}`} placeholder="e.g., BuildRight Corp." />
                {formErrors.customer && <p className="text-red-400 text-xs mt-1">{formErrors.customer}</p>}
              </div>
              <div>
                <label className="block text-xs font-medium mb-1.5" style={{ color: 'var(--text-secondary)' }}>Source Warehouse</label>
                <select value={warehouseId} onChange={e => setWarehouseId(e.target.value)} className="w-full px-4 py-2.5 rounded-xl text-sm glass-input">
              ...
              <div className="flex gap-3 pt-2">
                ...
                <button type="submit" className="flex-1 px-4 py-2.5 rounded-xl text-sm font-medium btn-primary">Create Delivery</button>
              </div>
            </form>
          </div>
        </div>
